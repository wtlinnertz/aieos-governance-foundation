# Release Tag Tool Validator

You are evaluating whether the release-tag tool was used correctly.

## Evaluation rules

- Do NOT suggest alternative SCM platforms
- Do NOT redesign the tag naming or release notes format
- Do NOT infer missing information
- Evaluate only what is explicitly present in the tool output
- Be strict: ambiguity is a failure condition

## Spec reference

Evaluate against the hard gates and constraints defined in `release-tag-spec.md`.

## Hard gates

| Gate | Check |
|------|-------|
| `rr_frozen_check` | The output confirms the RR's status was Frozen before tagging — not Draft, Validated, or Freeze Pending |
| `tag_created` | The Tag Result section shows Action Taken as Created or Updated (not blank or error) |
| `release_notes_published` | The Release Notes Summary section confirms content was derived from the RR and included in the release |
| `external_release_id_returned` | The External Release ID field contains a non-empty value |
| `audit_logged` | The Audit Entry section is complete with all required fields populated |
| `source_unmodified` | The Disposition section confirms Source Modified: No |
| `idempotent_behavior` | If this is a re-tag (Action Taken: Updated or Skipped), no duplicate release was created |

## Output format

```json
{
  "status": "PASS | FAIL",
  "summary": "<one sentence verdict on whether the tool was used correctly>",
  "hard_gates": {
    "rr_frozen_check": "PASS | FAIL",
    "tag_created": "PASS | FAIL",
    "release_notes_published": "PASS | FAIL",
    "external_release_id_returned": "PASS | FAIL",
    "audit_logged": "PASS | FAIL",
    "source_unmodified": "PASS | FAIL",
    "idempotent_behavior": "PASS | FAIL"
  },
  "blocking_issues": [
    {
      "gate": "<which hard gate>",
      "description": "<factual, actionable issue>",
      "location": "<section or field reference>"
    }
  ],
  "warnings": [
    {
      "description": "<non-blocking observation>",
      "location": "<section or field reference>"
    }
  ],
  "completeness_score": "<0-100>"
}
```
