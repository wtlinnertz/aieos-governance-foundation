...
**Last updated:** 2026-08-09 (the 2026-07-15 correction has aged in the other direction. Four of its five limitations closed between 2026-07-16 and 2026-07-26, and on 2026-08-07 a human froze a real kit artifact from the console in a browser. The three-driver claim now holds end to end. One limitation stands: validator calibration, FR-014. See Known limitations and the 2026-08-09 entry.)
...
v1.3 "Three Drivers" was cut internally on 2026-07-13. It delivers a shared governance engine — one freeze authority, one canonical state block, one cross-driver lock — plus a new autonomous driver, the dark factory. The goal is for sherpa, the console, and the dark factory to run as three drivers over that one engine. **That goal is now delivered and demonstrated.** The engine is verified against a live model. The console leg, the missing piece in July, is proven: the console loads all 15 real kits from `kit-manifest.yml`, and on 2026-08-07 a human clicked Freeze in a browser on a real EEK artifact and the harness wrote the one canonical FROZEN block. The caveat that remains is validator calibration (FR-014), which keeps a human at every promotion gate. See Known limitations below.
...
- **A canonical freeze representation.** `document-control.yaml` defines one Document Control block, with freeze status and validation outcome as orthogonal fields, plus a harness reader/writer and an instance validator. Since 2026-07-25 it is authoritative across the drivers: 69 of 78 kit templates emit it and the other 9 carry documented exemptions. The instance validator runs in every kit's CI and the console reads the block through a canonical reader (FR-018).
...
### Known limitations (found by real-model validation 2026-07-15, status as of 2026-08-09)

The v1.3 switch proof used a mock provider that returns a canned pass, so it exercised the plumbing without testing any place two real components meet. The first run against a real model and the real kits found the following. Four of the five closed between 2026-07-16 and 2026-07-26. Each entry keeps the original finding and adds the closure, because the finding is the useful part.

- **The console cannot yet load a real kit.** It required a per-kit `flow.yaml`, which no kit repo provides, while `kit-manifest.yml` sat there as the framework's declared machine-readable source of truth. **Closed 2026-07-26 (FR-023):** the console derives the flow for all 15 kits from `kit-manifest.yml`, the `flow.yaml` requirement is gone and the artifact-file convention contract resolves all 244 four-file paths. The switch was then demonstrated end to end in a browser on 2026-08-07 (see the 2026-08-09 entry).
- **Kit templates do not yet emit the canonical Document Control block.** 15 of 78 templates used the canonical `Artifact ID` field, the rest carried an older shape and a generated artifact could end up with two Document Control sections. **Closed 2026-07-25:** 69 templates migrated, 9 documented exemptions, the instance validator wired into all 15 kits' CI and the harness now updating the block in place so an artifact carries exactly one.
- **Five artifact types are not autonomously drivable.** The `prd`, `acf`, `dcf`, `dkr`, and `tdd` prompts required principles files as mandatory input, and the harness had no seam to supply them. **Closed 2026-07-26:** manifest 1.1 declares inputs per artifact (28 artifacts across 9 kits), the harness resolves them (framework inputs kit-relative and mandatory, human inputs initiative-relative and optional) and the console maps them into its required-inputs display. Entry-point artifacts still take a human brief by design.
- **Validator calibration is unresolved.** In back-to-back runs the same validator prompt and model returned opposite verdicts on one hard gate, and in one case failed an artifact for a requirement the spec explicitly exempts. FR-014 covers this, and it is a prerequisite for trusting autonomous promotion. **Still open as of 2026-08-09.** This is now the only item between the current state and unattended promotion, which is why every freeze gate keeps a human.
- **The harness needs explicit UTF-8 file I/O to run on Windows.** CI covered Linux only. **Closed 2026-07-16 (v1.3.1):** all 34 text-I/O sites declare `encoding="utf-8"`, a `windows-latest` CI job runs the suite and the first correct Windows run passed 435 tests with `PYTHONUTF8` unset.
...
### 2026-07-16 to 2026-08-09: v1.3 limitations closed and the console freeze leg proven in the browser

The work that turned the 2026-07-15 correction back around. Four of the five known limitations closed, then the console freeze leg ran for real with a human at the keyboard.

- [x] **v1.3.1 hardening** (2026-07-16). Windows support done properly: UTF-8 declared at all 34 text-I/O sites, a `windows-latest` CI job that caught the encoding gap within a minute of existing, 435 tests passing on Windows with `PYTHONUTF8` unset. Same patch series: truncated generations fail fast instead of burning the retry budget, generate and validate roles split so a model no longer grades its own work and freeze-before-promote enforcement actually wired in (it had tests and zero callers).
- [x] **FR-023** One flow definition (2026-07-26). The console derives all 15 kits from `kit-manifest.yml` and the per-kit `flow.yaml` requirement is retired. An artifact-file convention contract resolves all 244 four-file paths across the kits. Manifest 1.1 adds declared inputs per artifact, the harness resolves and hashes them into requests, and the console displays them as required inputs.
- [x] **Canonical Document Control everywhere** (2026-07-25). 69 templates migrated and 9 documented exemptions across 15 kit repos, 19 PRs merged in one day, validation green on real runners for every kit. The harness writes Owner, Frozen By, and Frozen Date, and updates the block in place so an artifact carries exactly one.
- [x] **Console freeze leg proven** (2026-08-07). A human clicked Freeze on a real EEK architecture document. The console shelled to the harness across the ADR-0003 seam, `sad.md` on disk reads `Status FROZEN` with the schema validator reporting one conformant block and the console renders the frozen state with the freeze control withdrawn. Getting there surfaced three console defects no unit test had caught, tracked internally as G-20/G-21/G-22: a flow page that fetched its own HTTP API behind a silent catch and rendered backend failure as a plausible empty flow, a step page rendering a placeholder while a fully built and tested StepView sat imported by nothing but its own test, and a freeze that succeeded on disk then failed to record itself. All three are fixed with route-level regression tests, the missing piece in each case being a test that renders the actual route. Console PRs #14, #15 and #16 merged. Suite at 342 passing, 2 skipped, tsc clean. The lesson worth exporting: a green suite is compatible with a feature that does not exist. Two of the three defects were finished, fully tested modules that nothing imported.
...
| ID | Item | Priority | Dependencies | Notes |
|----|------|----------|-------------|-------|
| **FR-014** | Prompt/model regression testing — add as Tier 3 test category and model-update trigger in healthcheck | High | — | Source: AG-002 (P116, PR63, PR68). No governance of prompt regression after model updates. Add prompt regression suite as Tier 3 category. Add model-update review trigger to healthcheck-playbook.md. As of 2026-08-09 this is the last open v1.3 limitation and the item that keeps a human at every promotion gate. |
