...
**Root cause:** The migration used CREATE INDEX (blocking) instead of CREATE INDEX CONCURRENTLY (non-blocking). The 8-minute lock blocked all writes. Also, the query planner immediately started using the partially-built index, degrading read performance.
